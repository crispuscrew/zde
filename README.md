...
password manager marks as a secret. Its safe sibling derives instead of storing:
`zde pass` is Argon2id over a master, a pepper file and a counter, and it prints
no password anywhere until the window that types one exists.
One keymap file generates the niri binds and the
...
you write one. pass's window, media, the modes beyond Normal,
