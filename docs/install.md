...
never takes the switch down with it: it arrives as a notification instead, one
for the whole switch, saying which apps did not start and what the runner said
about them. `journalctl --user -u zded` has every one of them whole, and `zde
doctor` says which of your desks name an app this machine cannot start before
you go there at all.
