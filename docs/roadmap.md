...
- security set: guest, panic + decoy, lock-preset, capture-block,
  ...
  `capture-block` writes niri a `block-out-from "screen-capture"` rule and
  reloads it, which covers the portal screencast, the wlr-screencopy tools and the
  two screenshot actions that take no selection - per app id and not per window,
  because that is all a niri rule can match ([`model.md`](model.md), section 6).
  The panic-lock variant
