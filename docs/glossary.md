...
| **picker** | the one surface that answers a question by listing what there is to choose: a list on the screen you are looking at, driven by up/down, `j`/`k`, or a digit. Desks on `Mod+Tab`, open windows on `Mod+w`, and desks again for the two verbs that send a window or a workspace to one you name, those two with a line on them saying what choosing will do |
